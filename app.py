...
st.set_page_config(page_title="룰루레몬 자동화 도구 [v19]", layout="wide")

# --- [로직] 1단계: 사용자 피드백 및 모든 히스토리 규칙 완벽 반영 ---
def build_perfect_key_v19(cid):
    ...
    # 3. 브랜드검색/브랜드존 (Strict: brandzone-na 필수) - [복구!]
    ...
    # 4. 검색광고 (ps_) 상세 로직 - 네이버 Generic 및 브랜드/제품 분류/prospecting 용어 통일
    ...
    # 6. 신규 DA 네이밍 규칙 (네이버da, 카카오da, 메타, 크리테오, 유튜브)
    # 형식: dm-[타겟]-[형식]-[캠페인키]
...
with tab1:
    st.header("어도비 데이터 통합 검수 (모든 지표 포함)")
    ...
    if files:
        ...
        full_adobe['AI_제안명'] = full_adobe['코드'].apply(build_perfect_key_v19)
