...
st.set_page_config(page_title="룰루레몬 자동화 도구 [v18]", layout="wide")

# --- [로직] 1단계: 사용자 피드백 및 모든 히스토리 규칙 완벽 반영 ---
def build_perfect_key_v18(cid):
    ...
    # 2. 카카오 옵트인 (kakaopn / transactional 구분) - [복구!]
    ...
    # 3. 검색광고 (ps_) 상세 로직 - 네이버 Generic 및 브랜드/제품 분류/용어(prospecting) 수정
    if cid_low.startswith("ps_"):
        ...
        # 네이버 Generic 형식: generic-middle-dm-prospecting-navermo
        if "keyword-generic" in cid_low:
            return f"generic-middle-dm-prospecting-{m}"
        
        # 브랜드존/브랜드검색 (Strict: brandzone-na 필수) - [복구!]
        ...
        # 카테고리 우선순위: keyword-brand가 있으면 무조건 brand
        ...
        t = "retargeting" if "retargeting" in cid_low else "prospecting" # pro -> prospecting 통일
    ...
    # 5. 신규 DA 네이밍 규칙 (네이버da, 카카오da, 메타, 크리테오, 유튜브)
    # 형식: dm-[타겟]-[형식]-[캠페인키]
...
with tab1:
    st.header("어도비 통합 검수 (Cart Adds 포함)")
    ...
    if files:
        ...
        full_adobe['AI_제안명'] = full_adobe['코드'].apply(build_perfect_key_v18)
...
with tab2:
    ...
    if adobe_in and media_ins:
        ...
        if st.button("🚀 최종 통합 실행"):
            ...
            final['일'] = final['일'].fillna(final['날짜']); final['캠페인명'] = final['캠페인명'].fillna(final['AI_제안명'])
            st.dataframe(final.drop(columns=['날짜', 'AI_제안명']).sort_values(['일', '캠페인명']).head(1000))
            st.download_button("📥 리포트 다운로드", final.to_csv(index=False).encode('utf-8-sig'), "lululemon_final_report.csv")
