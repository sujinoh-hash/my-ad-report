...
st.set_page_config(page_title="룰루레몬 자동화 도구 [v20]", layout="wide")

# --- [로직] 1단계: 브랜드검색 및 모든 히스토리 규칙 완벽 반영 ---
def build_perfect_key_v20(cid):
    ...
    # 1. [최우선] 브랜드검색/브랜드존 로직 (ps_ 보다 먼저 검사해야 함)
    # 규칙: brandzone 단어가 있고 brandzone-na가 포함되어야 함
    ...
    # 3. 카카오 옵트인 (kakaopn / transactional 구분)
    ...
    if any(x in cid_low for x in ["dsp_naver", "dsp_kakao", "meta", "smp_fbig", "criteo", "dsp_yt"]):
        ...
        # 캠페인 키 판별 (Golf, Springstory, Run, Train, Pants 등)
...
with tab1:
    ...
    if files:
        ...
        full_adobe['AI_제안명'] = full_adobe['코드'].apply(build_perfect_key_v20)
        ...
        st.download_button("📥 검수 완료 다운로드", full_adobe.to_csv(index=False).encode('utf-8-sig'), "adobe_checked.csv")
