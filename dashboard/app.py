"""
Streamlit 대시보드 - 키워드 검색량 + 블로그/카페 + 유튜브 수집
"""
import os
import time
import requests
...
@st.cache_data(ttl=3600, show_spinner=False)
def fetch_youtube_comments(video_id, max_results=30):
    api_key = os.environ.get("YOUTUBE_API_KEY", "")
...
def build_prompt(keywords, stats, posts, top_kws, yt_videos, yt_comments):
    stats_text = "\n".join([
    ...
    yt_text = "\n".join([
        f"- {v['title']} ({v['channel']}, {v['published']})"
        for v in yt_videos[:10]
    ]) if yt_videos else "없음"
    comment_text = "\n".join([
        f"- {c['text']}"
        for c in yt_comments[:15]
    ]) if yt_comments else "없음"
...
if run_btn and keyword_input.strip():
    ...
    with st.spinner("📝 블로그/카페 수집 중..."):
        all_posts_raw = []
        for kw in keywords:
            all_posts_raw.extend(fetch_blog_cafe(kw))
        all_posts = filter_by_date(all_posts_raw, date_from, date_to)

    yt_videos = []
    yt_comments = []
    if use_youtube:
        with st.spinner("▶️ 유튜브 영상 수집 중..."):
            for kw in keywords:
                yt_videos.extend(fetch_youtube_videos(kw, max_results=20, date_from=date_from, date_to=date_to))

        with st.spinner("▶️ 유튜브 댓글 수집 중..."):
    ...
    st.success(f"✅ 수집 완료! 키워드 {len(stats)}개 / 게시글 {len(all_posts)}건 / 유튜브 {len(yt_videos)}건 / 댓글 {len(yt_comments)}건")
    ...
    with tab2:
        if not all_posts:
            st.info(f"{date_from} ~ {date_to} 기간 내 수집된 게시글이 없습니다.")
        else:
            st.caption(f"출처: 네이버 Open API · 최신순 · {date_from} ~ {date_to} · {len(all_posts)}건")
            ...
            if top_kws:
                ...
                st.caption("블로그/카페 게시글 제목 + 내용 기준")
    ...
    with tab3:
        if not has_youtube:
            ...
            st.caption(f"출처: YouTube Data API v3 · 조회수 순 · {date_from} ~ {date_to}")
            ...
            with col_c:
                st.markdown("**💬 인기 댓글 (상위 5개 영상)**")
                if yt_comments:
                    for c in yt_comments[:20]:
                        st.markdown(
                            f"<small style='color:#333'>{c['text']}</small>  \n"
                            f"<small style='color:#aaa'>👍 {c['likes']} · {c['published']}</small>",
                            unsafe_allow_html=True
                        )
                        st.markdown("---")
                else:
            ...
            st.download_button("📥 유튜브 CSV", df_yt.to_csv(index=False, encoding="utf-8-sig"),
                file_name=f"유튜브_{datetime.now().strftime('%Y%m%d')}.csv", mime="text/csv")
    ...
    with tab4:
        ...
        prompt = build_prompt(keywords, stats, all_posts, top_kws, yt_videos, yt_comments)
